...
for p in \
  ...
  #"$p\\rc.exe" -i "$msklc\\inc" -DSTD_CALL -DCONDITION_HANDLING=1 -DNT_UP=1 -DNT_INST= -DWIN32=1 -D_NT1X_=1 -DWINNT=1 -D_WIN32_WINNT=x5 -DWINVER=x4 -D_WIN32_IE=x4 -DWIN32_LEAN_AND_MEAN=1 -DDEVL=1 -DFPO=1 -DNDEBUG -l "$localeid" -fo "$TEMP_WIN\\my_layout.res" "${kbdname}.rc"
  "$p\\rc.exe" -i "$msklc\\inc" -DSTD_CALL -DCONDITION_HANDLING=1 -DNT_UP=1 -DNT_INST= -DWIN32=1 -D_NT1X_=1 -DWINNT=1 -D_WIN32_WINNT=0x0500 -DWINVER=0x0400 -D_WIN32_IE=0x0400 -DWIN32_LEAN_AND_MEAN=1 -DDEVL=1 -DFPO=1 -DNDEBUG -l "$localeid" -fo "$TEMP_WIN\\my_layout.res" "${kbdname}.rc"
  ...
  # FIXME -Gz (x86 only)
  # FIXME -Gm
  "$p\\cl.exe" -nologo "-I$msklc\\inc" -DNOGDICAPMASKS -DNOWINMESSAGES -DNOWINSTYLES -DNOSYSMETRICS -DNOMENUS -DNOICONS -DNOSYSCOMMANDS -DNORASTEROPS -DNOSHOWWINDOW -DOEMRESOURCE -DNOATOM -DNOCLIPBOARD -DNOCOLOR -DNOCTLMGR -DNODRAWTEXT -DNOGDI -DNOKERNEL -DNONLS -DNOMB -DNOMEMMGR -DNOMETAFILE -DNOMINMAX -DNOMSG -DNOOPENFILE -DNOSCROLL -DNOSERVICE -DNOSOUND -DNOTEXTMETRIC -DNOWINOFFSETS -DNOWH -DNOCOMM -DNOKANJI -DNOHELP -DNOPROFILER -DNODEFERWINDOWPOS -DNOMCX -DWIN32_LEAN_AND_MEAN -DRoster -DSTD_CALL -D_WIN32_WINNT=0x0500 $defwow64 -DWINVER=0x0500 -D_WIN32_IE=0x0500 -MD -c -Zp8 -Gy -W3 -WX -EHs-c- -GR- -GF -Z7 -Ox -Fo"$TEMP_WIN\\my_layout.obj" "${kbdname}.c"
  ...
  # FIXME -noentry will probably disable main(); we need main() though
  #   -section:init,d might have deleted it :(
  ...
  # FIXME -SECTION:INIT,d -noentry
  # FIXME -merge:.rdata=.data
  "$p\\link.exe" "-machine:$machine" -nologo -OPT:REF -OPT:ICF -dll "-libpath:$msklc\\lib\\$libdir" "-subsystem:native,$subsys_native_ver" -merge:.rdata=.text -PDBPATH:NONE -STACK:4096,1024 -opt:nowin98 "-version:$my_ver" -release -merge:.edata=.data -merge:.text=.data -merge:.bss=.data -section:.data,re -MERGE:_PAGE=PAGE -MERGE:_TEXT=.text-merge:.edata=.data -merge:.srdata=.data -merge:.text=.data -merge:.bss=.data -section:.data,re -MERGE:_PAGE=PAGE -MERGE:_TEXT=.text "-def:${kbdname}.def" "-out:$outdir\\${kbdname}.dll" "$TEMP_WIN\\my_layout.res" "$TEMP_WIN\\my_layout.obj"

  printf '\nHOWK22\n\n'

  [ -n "$(ls -1 "$TEMP_WIN\\my_layout."*)" ] && rm "$TEMP_WIN\\my_layout."*
done
