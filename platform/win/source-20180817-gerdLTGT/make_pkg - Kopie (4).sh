...
run_elevated()(
  ...
  readonly t="$(mktemp run_elevated.XXXXXXXXXXXXXXXX).vbs"
...
if [ -n "$(wmic product where name="$productname_quoted")" ]; then
  # win apps only sometimes accept forward slashes
  readonly a="$(printf %s "$TMP/app.cmd" | sed -r 's|/|\|g')"
...
cat > "$a" <<END
del /f /q C:\\Windows\\System32\\$kbdname
del /f /q C:\\Windows\\SysWOW64\\$kbdname
END
  # FIXME
  printf '%s\n' "DEBUG run_elevated $a"
